...
class Decoder(nn.Module):
    ...
    def decode(self, g, n_sampled_points):
        ...
        for i, decoder in enumerate(self.decoder):
            ...
            flow_out['p_prior_samples'] = [self.reparameterize(flow_out['p_prior_mus'][0], flow_out['p_prior_logvars'][0])]
        ...
        samples = torch.zeros_like(g.shape[0], self.input_dim, n_sampled_points)
        labels = torch.zeros(g.shape[0], n_sampled_points)
        for t in range(self.n_flows):
