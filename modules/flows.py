...
class PriorFlow(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.seq_len = cfg.vae.latent_dim // self.e_dim  # number of tokens in the sequence
        ...
        # Learnable absolute positional embedding: tokens are Hilbert/Z-order
        # serialized, so sequence position carries real spatial meaning that
        # the permutation-equivariant attention blocks can't otherwise see.
        self.pos_emb = nn.Parameter(torch.randn(1, self.seq_len, self.width) * 0.02)
    ...
    def forward(self, x, t):
        ...
        x = x + self.pos_emb              # inject sequence position
