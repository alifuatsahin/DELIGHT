...
class EfficientSpatialQKVAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        assert C % self.n_heads == 0
        ch = C // self.n_heads
        q = q.reshape(B, self.n_heads, ch, N).permute(0, 1, 3, 2).contiguous()  # [B, N, n_heads, ch]
        k = k.reshape(B, self.n_heads, ch, M).permute(0, 1, 3, 2).contiguous()
        v = v.reshape(B, self.n_heads, ch, M).permute(0, 1, 3, 2).contiguous()
        scale = 1 / math.sqrt(ch)
...
class EfficientQKVAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        assert C % self.n_heads == 0
        ch = C // self.n_heads
        q = q.reshape(B, self.n_heads, N, ch)
        k = k.reshape(B, self.n_heads, M, ch)
        v = v.reshape(B, self.n_heads, M, ch)
        scale = 1 / math.sqrt(ch)
        y = F.scaled_dot_product_attention(q, k, v, scale=scale)
